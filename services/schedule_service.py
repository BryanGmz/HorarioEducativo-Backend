...
def __get_schedule_by_qualifications__(schedule_manager:ScheduleManager, assignment_manager:AssignmentManager, priority:int, db:Session):
...
def get_schedule(db:Session, generate_schedule:GenerateSchedule):
    ...
    schedule_by_priorities = [
        __get_schedule_by_qualifications__(schedule_manager, assignment_manager, 1, db),
        __get_schedule_by_qualifications__(__create_schedule_manager__(generate_schedule, db), __create_assignment_manager__(db), 2, db),
        __get_schedule_by_qualifications__(__create_schedule_manager__(generate_schedule, db), __create_assignment_manager__(db), 3, db)
    ]
    return {
